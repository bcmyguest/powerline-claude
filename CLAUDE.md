...
- **Themes:** built-in palettes live in `theme.rs::PALETTES`, stored as `[SegmentColors; 8]` indexed by the `Family` enum (including `context_warn`/`context_alert` — the orange/red backgrounds the context segment switches to at 80k/125k tokens); `catppuccin-mocha` (`PALETTES[0]`) is the fallback for every unspecified custom-theme value. A custom theme is a directory containing `theme.yaml` (any subset of the eight families), selected by passing a path to `--theme`. `docs/themes/synthwave` is the documented example. `stats`, `effort`, and `usage` have no palette families of their own — their registry rows borrow `cost`-on-`context`, `model`, and `cost` respectively; `theme.rs` never mentions a segment by name.
