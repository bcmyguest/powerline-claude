...
**`segments.rs::MODULES` is the segment registry**: one table row per module defining its CLI name, default bar position, overflow-drop priority (when the bar exceeds the terminal width, `run` sheds the lowest-priority segments until it fits, always keeping at least one), and which palette families paint it. Parsing, the default `--modules` value in `cli.rs`, and theming all derive from it — adding a module is one registry row plus one formatting arm in `segment_texts` (plus docs; `tests/docs.rs` fails until they're synced).
...
- **Themes:** built-in palettes live in `theme.rs::PALETTES`, stored as `[SegmentColors; 8]` indexed by the `Family` enum (including `context_warn`/`context_alert` — the orange/red backgrounds the context segment switches to at 80k/125k tokens, reused by the usage segment when the tightest rate-limit window drops under 20%/5% remaining); `catppuccin-mocha` (`PALETTES[0]`) is the fallback for every unspecified custom-theme value. A custom theme is a directory containing `theme.yaml` (any subset of the eight families), selected by passing a path to `--theme` — or a bare name, which resolves against `~/.config/powerline-claude/themes/<name>` after the built-ins (built-in names always win). `docs/themes/synthwave` is the documented example. `stats`, `effort`, and `usage` have no palette families of their own — their registry rows borrow `cost`-on-`context`, `model`, and `cost` respectively; `theme.rs` never mentions a segment by name.
